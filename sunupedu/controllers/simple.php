...
class Simple extends IController {
    ...
    function is_login() {
        echo JSON::encode(ISafe::get('user_id'));
    }
...
//        }
        ...
        $addressList = $addressObj->query('user_id = ' . $user_id.' AND `default` =1 ');
        if(!$addressList){
            IError::show(403, '请添加默认地址');
                exit;
        }
        //更新$addressList数据
        foreach ($addressList as $key => $val) {
            $temp = area::name($val['province'], $val['city'], $val['area'], $val['school']);
            //,$val['grade'],$val['class']
            if (isset($temp[$val['province']]) && isset($temp[$val['city']]) && isset($temp[$val['area']]) && isset($temp[$val['school']])) {
                ...
                $addressList[$key]['school_val'] = $temp[$val['school']];
                //	$addressList[$key]['grade_val']     = $temp[$val['grade']];
                //	$addressList[$key]['class_val']     = $temp[$val['class']];
...
//        }
    ...
    function address_add() {
        ...
        $city = IFilter::act(IReq::get('city'), 'int');
        $area = IFilter::act(IReq::get('area'), 'int');
        $school = IFilter::act(IReq::get('school'), 'int');
        $grade = IFilter::act(IReq::get('grade'));
        $class = IFilter::act(IReq::get('class'));
        $address = IFilter::act(IReq::get('address'));
        $zip = IFilter::act(IReq::get('zip'));
        ...
        if ($addressRow) {
            ...
            //获取地区text
            $areaList = area::name($province, $city, $area, $school);
            ...
            $sqlData['school_val'] = $areaList[$school];
            $sqlData['grade'] = $areaList[$grade];
            $sqlData['class'] = $areaList[$class];

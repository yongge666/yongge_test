		...
		<div class="box clearfix">
			<form action='<?php echo IUrl::creatUrl("/simple/reg_act");?>' method='post'>
				...
				<table class="form_table f_l">
					<col width="260px" />
					<col />
					<tr><th>邮箱：</th><td><input class="gray" type="text" name='email' pattern="email" alt="填写正确的邮箱格式" />
					<label>邮箱格式：123456@qq.com</label></td></tr>
					...
					<tr><th>确认密码：</th><td><input class="gray" type="password" name='repassword' pattern="^\S{6,32}$" bind='password' alt='重复上面所填写的密码' />
					<label>重复上面所填写的密码，<b style="color:#FF0000">请牢记用户名及密码，以便以后登录使用。<b></label></td></tr>
...
<tr style="color:#FF0000"><th style="color:#FF0000">*注意：</th><td>
					以下信息如填写不准确或不真实，造成错误或损失由用户负责。
</td></tr>
				...
				<tr>
                   ...
                   <th>选择学生学校地址：</th>
                                                <td> <select  style="width:70px" name="province" child="city,area,school" onchange="areaChangeCallback(this);"></select>
						...
						<select  style="width:100px" name="school" parent="area" pattern="required"></select></td>
...
<th>选择学生年级与班级：</th>
<td>
<select name='grade' pattern="required">
...
<select name='class' pattern="required">
...
<option value='40班'>40班</option>
					...
					<tr><th>学生姓名：</th><td><input name='accept_name' class="normal" pattern='required' alt='学生姓名不能为空' type="text" /></td></tr>

<tr>
<th>家长手机：</th>
<td><input name='mobile' class="normal" pattern="required" empty alt='手机号码格式不正确' type="text" /></td>
</tr>
				...
				<tr hidden="hidden">
					...
					<td>设为默认</td><td> <label><input name='default' type='checkbox' checked = 'checked' value='1'></label></td>
				</tr>

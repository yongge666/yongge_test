	...
	<div class="content form_content">
		<form action='<?php echo IUrl::creatUrl("/tools/article_edit_act");?>' method='post' name='article'>
			<table class="form_table">
				...
				<input type='hidden' name='relation_goods' value='<?php echo isset($this->relationStr)?$this->relationStr:"";?>' />
				<tr>
					<th>分类：</th>
					<td>
					...
					<td>
						<div id='goods_box' class='photo_list'>
							<?php if(!empty($this->goodsList)){?>
								...
								<img src='<?php echo IUrl::creatUrl("")."".$item['img']."";?>' alt='<?php echo isset($item['name'])?$item['name']:"";?>' width='120px' />
								<?php }?>
							<?php }?>
						</div>
...
function relationCallBack(content)
{
	...
	$(content['data']).each(
		function(i)
		{
			...
			dataHTML+=' <img width="120px" src="<?php echo IUrl::creatUrl("")."";?>'+content['data'][i]['img']+'" alt="'+content['data'][i]['name']+'" /> ';
		}
	);
	//动态插入图片

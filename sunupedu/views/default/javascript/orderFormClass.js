...
function orderFormClass()
{
	...
	this.addressInit = function(defaultAddressId)
	{
		if(defaultAddressId)
		{
			...
			$('input[name="radio_address[]"][value="'+defaultAddressId+'"]').trigger('click');
			this.addressSave();
		}
		else
		{
			$('input[name="otheraddr"]').trigger('click');
		}
	}
	...
	this.checkAddr = function(obj){
		...
		var name = thisObj.attr("addr_name");
		var area = thisObj.attr("addr_area");
		//匹配其他除本身已经选中的值
		var bool = true;
		selObj.not(thisObj).each(function()
		{
			//校验要求: N个地址 区必须相同 , 姓名 不能相同
			var eachName = $(this).attr('addr_name');
			var eachArea = $(this).attr('addr_area');
			if(eachName == name){
				alert("收货人姓名不能相同");
	 ...
	 */
	this.addressSelected = function(jsonData,obj)
	{
		//去掉其他收货地址选择
		$('input[name="otheraddr"]').attr("checked",false);
		//取消复选按钮do nothing
		if( !$(obj).is(":checked") ){
			//this.addressEmpty();
			return false;
		...
		{ 
			str += "<tr><th>收货人姓名：</th><td>" +$(this).attr("addr_name")+" </td>  <th> 地址：</th><td> " +$(this).attr("addr")+" </td></tr>";
		});
		$('#addressShowBox').html(str);
	...
	this.addressModToggle = function()
	{
		...
		switch(toggleMod)
		{
			...
			case "exit":
			{
				...
				{
					//$('input[name="radio_address[]"][value="'+this.addressActiveId+'"]').trigger('click');
				}
	...
	this.addressSave = function()
	{
		...
		this.addressActiveId = $('input[name="radio_address[]"]:checked').val();

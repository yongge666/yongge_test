...
class Util
{
	...
	public static function search($search)
	{
		...
		//支付时间处理
		if(isset($search['pay_time_start']) && $search['pay_time_start'])
		{
			$where[] = "`pay_time` >= '".$search['pay_time_start']."' ";
		}
		
		if(!isset($search['pay_time_end']) || empty($search['pay_time_end']))
		{
			$search['pay_time_end'] = date("Y-m-d 23:59:59");
		} 
		$where[] = "`pay_time` <= '".$search['pay_time_end']."' ";
		unset($search['pay_time_end']);
		unset($search['pay_time_start']);

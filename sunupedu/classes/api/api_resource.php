...
return array(
	...
	'getGoodsInfo' => array(
		'query' => array(
			...
			'fields' => 'go.name,go.id as goods_id,go.img,go.sell_price,go.point,go.weight,go.store_nums,go.exp,go.goods_no,0 as product_id',
			'type'   => 'row',
		)
	),
	//取货品数据
	'getProductInfo' => array(
		'query' => array(
			...
			'fields' => 'pro.sell_price,pro.weight,pro.id as product_id,pro.spec_array,pro.goods_id,pro.store_nums,pro.products_no as goods_no,go.name,go.point,go.exp,go.img',
			'type'   => 'row',
		)
	),
	//取文章置顶列表
	...
	//同区域新品列表
	'getCommendNewSameArea' => array(
			'query' => array(
					'name' => 'commend_goods as co',
					'join' => 'left join goods as go on co.goods_id = go.id left join seller as se on go.seller_id = se.id',
					'where' => 'co.commend_id = 1 and go.seller_id > 0 and se.area = #area_val# and go.is_del = 0 AND go.id is not null',
					'fields' => 'go.img,go.sell_price,go.name,go.id,go.market_price',
					'limit'=>'8',
	...
	//同区热卖商品列表
	'getCommendHotSameArea' => array(
			'query' => array(
					'name' => 'commend_goods as co',
					'join' => 'left join goods as go on co.goods_id = go.id left join seller as se on go.seller_id = se.id',
					'where' => 'co.commend_id = 3 and go.seller_id > 0 and se.area = #area_val# and go.is_del = 0 AND go.id is not null',
					'fields' => 'go.img,go.sell_price,go.name,go.id',
					'limit'=>'8',
					'order'=>'go.sort asc,go.id desc'
			)
	...
	//根据商品分类取得同区域商品列表
	'getCategoryExtendListSameArea'=>array(
			'query'=>array(
					'name'  => 'category_extend as ca',
					'fields'=> 'go.*',
					'join'  => 'left join goods as go on go.id = ca.goods_id left join seller as se on go.seller_id = se.id ',
					'where' => ' ca.category_id = #categroy_id# and go.seller_id > 0 and se.area = #area_val# and go.is_del = 0',
					'order' => ' go.sort asc,go.id desc',
					'limit' => 100,
			)
	),
